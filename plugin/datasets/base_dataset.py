...
class BaseMapDataset(Dataset):
    ...
    def format_results(self, results, denormalize=True, prefix=None, save_semantic=False):
        ...
        if output_format == 'raster':
            ...
            for pred in results:
                ...
                attr_logits_case = pred.get('attr_logits', {})
                    ...
                    for i in range(len(pred['scores'])):
                        ...
                        for attr_name, attr_values in attr_preds_case.items():
                            if len(attr_values) > i:
                                single_case['attr_preds'].setdefault(attr_name, []).append(
                                    int(attr_values[i]))
                        for attr_name, attr_values in attr_logits_case.items():
                            if len(attr_values) > i:
                                attr_value = attr_values[i]
                                if isinstance(attr_value, np.ndarray):
                                    attr_value_serial = attr_value.tolist()
                                elif isinstance(attr_value, torch.Tensor):
                                    attr_value_serial = attr_value.detach().cpu().tolist()
                                else:
                                    attr_value_serial = attr_value
                                single_case['attr_logits'].setdefault(attr_name, []).append(attr_value_serial)
                else:
                    single_case['attr_preds'] = {attr_name: [] for attr_name in attr_preds_case.keys()}
                    single_case['attr_logits'] = {attr_name: [] for attr_name in attr_logits_case.keys()}

                if single_case['attr_preds']:
                    for attr_name, values in single_case['attr_preds'].items():
                        single_case['attr_preds'][attr_name] = [int(v) for v in values]
                if single_case['attr_logits']:
                    for attr_name, values in single_case['attr_logits'].items():
                        single_case['attr_logits'][attr_name] = [
    ...
    def evaluate(self, results, logger=None, **kwargs):
        ...
        if result_path is None:
            raise RuntimeError('format_results must return the path to the saved predictions.')
    ...
    def show_gt(self, idx, out_dir='demo/'):
        ...
        if 'vectors' in data:
            ...
            specified_path = osp.join(out_dir, f'map_{idx}.jpg')
    ...
    def __getitem__(self, idx):
        ...
        Ensures training samples have at least one vector GT by resampling
        a few times if needed. Validation/test samples may not include vectors.
        ...
        while attempt < max_tries:
            input_dict = self.get_sample(curr_idx)
            data = self.pipeline(input_dict)
            if data is None:
                # pick another sample if a transform filtered this one
                curr_idx = np.random.randint(0, len(self.samples))
                attempt += 1
                continue
            # If vectors exist, make sure there is at least one GT
            if 'vectors' in data:
                vecs = data['vectors']
                if isinstance(vecs, DC):
                    vecs = vecs.data
                total = 0
                for class_vectors in vecs.values():
                    total += len(class_vectors)
                if total == 0:
                    # Warn with helpful context for debugging empty patches
                    warnings.warn(
                        'Empty vector set in ROI; resampling. '
                        f"token={input_dict.get('token')}, "
                        f"scene={input_dict.get('scene_name')}, "
                        f"frame={input_dict.get('frame_number')}, "
                        f"lat_long_heading={input_dict.get('lat_long_heading')}",
                        RuntimeWarning
                    )
                    curr_idx = np.random.randint(0, len(self.samples))
                    attempt += 1
        ...
        if data is None:
            # last resort: use original index
            input_dict = self.get_sample(idx)
            data = self.pipeline(input_dict)

        # prepare the local sequence index info
        seq_info = self.idx2scene[curr_idx]
        # Only expose top-level `seq_info` in test/val to avoid
        # batch-collate key mismatches during training.
        if getattr(self, 'test_mode', False):
            data['seq_info'] = DC(seq_info, cpu_only=True)
        # Also tuck it into img_metas for universal access
        if isinstance(data.get('img_metas', None), DC):
            try:
                data['img_metas'].data['seq_info'] = seq_info
            except Exception:
                pass
        ...
        if self.multi_frame: # used when sampling multi-frame training data
            ...
            min_idx = max(0, local_idx_curr - span)
            # sample indices within [min_idx, local_idx_curr)
            window_len = max(local_idx_curr - min_idx, 1)
            want = self.multi_frame - 1
            replace_flag = window_len < want
            sampled_offsets = np.random.choice(window_len, want, replace=replace_flag).tolist()
            sampled_offsets = sorted(sampled_offsets)
            local_indices_prev = [local_idx_curr - 1 - x for x in sampled_offsets]
            local_indices_prev = [x if x>=0 else 0 for x in local_indices_prev]

            data['img_metas'].data['local_idx'] = local_idx_curr

            all_prev_data = []
            used_local_indices_prev = []
            for target_local_idx in local_indices_prev:
                # Try to find a previous frame within the window that has vectors
                max_prev_tries = 8
                tries = 0
                actual_local_idx = target_local_idx
                data_prev = None
                while tries < max_prev_tries:
                    global_idx_prev = scene_seq_info[0] + actual_local_idx
                    input_dict_prev = self.get_sample(global_idx_prev)
                    dp = self.pipeline(input_dict_prev)
                    vecs_prev = dp.get('vectors', None)
                    if isinstance(vecs_prev, DC):
                        vecs_prev = vecs_prev.data
                    total_prev = 0
                    if isinstance(vecs_prev, dict):
                        for vv in vecs_prev.values():
                            total_prev += len(vv)
                    if total_prev > 0:
                        data_prev = dp
                        break
                    # pick another previous index within window [min_idx, local_idx_curr)
                    warnings.warn(
                        'Empty prev-frame vectors; retrying another previous frame. '
                        f"scene={scene_name}, curr_local_idx={local_idx_curr}, tried_local_idx={actual_local_idx}",
                        RuntimeWarning
                    )
                    if local_idx_curr - 1 > min_idx:
                        actual_local_idx = np.random.randint(min_idx, local_idx_curr)
                    tries += 1

                if data_prev is None:
                    # Fallback: build a fresh copy from current sample to avoid
                    # cyclic references and key mismatches during collate.
                    # Shallow-copy the dict and clone img_metas payload only.
                    data_prev = dict(data)
                    if isinstance(data_prev.get('img_metas', None), DC):
                        meta_copy = deepcopy(data_prev['img_metas'].data)
                        data_prev['img_metas'] = DC(meta_copy, cpu_only=True)
                    actual_local_idx = local_idx_curr

                data_prev['img_metas'].data['local_idx'] = actual_local_idx
                all_prev_data.append(data_prev)
                used_local_indices_prev.append(actual_local_idx)

            all_local2global_info = []
            if self.matching:
                scene_matching_info = self.matching_meta[scene_name]
                for local_idx_prev in used_local_indices_prev:
            ...
            # Keep as list[dict] so mmcv.collate transposes over time:
            # per-sample list -> per-timestep dict-of-batch.
            data['all_prev_data'] = all_prev_data
            data['all_local2global_info'] = all_local2global_info
        # Ensure collate sees consistent keys across the batch
        if 'all_prev_data' not in data:
            data['all_prev_data'] = []
        if 'all_local2global_info' not in data:
            data['all_local2global_info'] = []
