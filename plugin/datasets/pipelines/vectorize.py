...
class Vector(np.ndarray):
    """An ndarray that carries a dict of attributes on the side."""
    def __new__(cls, arr: np.ndarray, attrs: dict):
        # view the plain array as our subclass
        obj = np.asarray(arr).view(cls)
        obj.attrs = attrs
        return obj

    def __array_finalize__(self, obj):
        # ensure attrs survives slicing, views, etc.
        if obj is None: 
            return
        self.attrs = getattr(obj, 'attrs', {})

    def __reduce__(self):
        """Ensure attrs survive pickle/unpickle (e.g., between dataloader workers)."""
        pickled_state = super().__reduce__()
        state = pickled_state[2] + (self.attrs,)
        return pickled_state[0], pickled_state[1], state
    ...
    def get_vectorized_lines(self, map_geoms: Dict) -> Dict:
        ...
        for label, geom_list in map_geoms.items():
            ...
            for i, geom in enumerate(geom_list):
                attrs = getattr(geom_list, "attrs", None)
                if attrs:
                    raw_attrs = attrs[i]
                else:
                    raw_attrs = {}
                if geom.geom_type == 'LineString':
                    ...
                    vectors[label].append(Vector(line, raw_attrs))

                elif geom.geom_type == 'Polygon':
                    # ————————————
                    # sample the exterior boundary
                    if self.simplify:
                        # simplify the whole polygon first
                        poly = geom.simplify(0.2, preserve_topology=True)
                        ext_coords = np.array(poly.exterior.coords)
                    else:
                        # sample along the exterior ring
                        ext_ring = LineString(geom.exterior.coords)
                        ext_coords = self.sample_fn(ext_ring)
                    ext_coords = ext_coords[:, :self.coords_dim]
                    if self.normalize:
                        ext_coords = self.normalize_line(ext_coords)
                    if self.permute:
                        ext_coords = self.permute_line(ext_coords)
                    vectors[label].append(Vector(ext_coords, raw_attrs))

                    # sample any interior holes
                    for interior in geom.interiors:
                        int_ring = LineString(interior.coords)
                        if self.simplify:
                            # simplify the ring directly
                            int_ring = LineString(
                                int_ring.simplify(0.2, preserve_topology=True).coords
                            )
                            int_coords = np.array(int_ring.coords)
                        else:
                            int_coords = self.sample_fn(int_ring)
                        int_coords = int_coords[:, :self.coords_dim]
                        if self.normalize:
                            int_coords = self.normalize_line(int_coords)
                        if self.permute:
                            int_coords = self.permute_line(int_coords)
                        vectors[label].append(Vector(int_coords, raw_attrs))
