...
@TRANSFORMS.register_module(force=True)
class CropTopMultiViewImages(object):
    """Crop the top portion of each camera image and update intrinsics.

    Args:
        crop_ratio (float): Fraction in [0, 1) of image height to remove
            from the top. Defaults to 0 (no crop).
        change_intrinsics (bool): Whether to adjust intrinsics/ego2img.
    """

    def __init__(self, crop_ratio=0.0, change_intrinsics=True):
    ...
    def __call__(self, results):
        if self.crop_ratio <= 0 or 'img' not in results:
            return results
        ...
        for img in results['img']:
            h, w = img.shape[:2]
            offset = int(round(h * self.crop_ratio))
            if offset <= 0:
                new_imgs.append(img)
            ...
            if offset >= h:
                offset = h - 1
            new_imgs.append(img[offset:, :])
            crop_offsets.append(offset)

        results['img'] = new_imgs
        new_shapes = [img.shape for img in new_imgs]
        results['img_shape'] = new_shapes
        results['pad_shape'] = new_shapes

        if self.change_intrinsics and 'cam_intrinsics' in results and 'ego2img' in results:
            post_intrinsics, post_ego2imgs = [], []
            for K, ego2img, offset in zip(results['cam_intrinsics'],
                                          results['ego2img'], crop_offsets):
                if offset == 0:
                    post_intrinsics.append(K)
                    post_ego2imgs.append(ego2img)
                    continue
                K_new = np.array(K, copy=True)
                K_new[1, 2] = K_new[1, 2] - offset
                post_intrinsics.append(K_new)

                ego2img_arr = np.array(ego2img, copy=True)
                translate = np.eye(4, dtype=ego2img_arr.dtype)
                translate[1, 2] = translate[1, 2] - offset
                post_ego2imgs.append(translate @ ego2img_arr)

            results['cam_intrinsics'] = post_intrinsics
            results['ego2img'] = post_ego2imgs

        return results

    def __repr__(self):
        return (f'{self.__class__.__name__}(crop_ratio={self.crop_ratio}, '
                f'change_intrinsics={self.change_intrinsics})')
