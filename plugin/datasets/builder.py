...
from plugin.utils.data_container import collate as dc_collate
from torch.utils.data import DataLoader

from .samplers.group_sampler import GroupSampler, DistributedGroupSampler
...
def build_dataloader(dataset,
    ...
    # True entry here!!!
    runner_type = runner_type or dict(type='EpochBasedRunner')
    if (runner_type.get('type') == 'IterBasedRunner'
            and isinstance(shuffler_sampler, dict)
            and shuffler_sampler.get('type') == 'InfiniteGroupEachSampleInBatchSampler'):
    ...
    data_loader = DataLoader(
        ...
        collate_fn=partial(safe_collate, samples_per_gpu=samples_per_gpu),
        pin_memory=False,
...
def safe_collate(batch, samples_per_gpu=1):
    try:
        if isinstance(batch, (list, tuple)) and batch and isinstance(batch[0], Mapping):
            normalized = []
            for sample in batch:
                if isinstance(sample, Mapping):
                    nd = dict(sample)
                    nd.setdefault('all_prev_data', [])
                    nd.setdefault('all_local2global_info', [])
                    normalized.append(nd)
                else:
                    normalized.append(sample)
            return dc_collate(normalized, samples_per_gpu)
        return dc_collate(batch, samples_per_gpu)
    except KeyError:
        fixed = []
        for sample in batch:
            if isinstance(sample, Mapping):
                ...
                fixed.append(sample)
        return dc_collate(fixed, samples_per_gpu)
