...
def instance_match(pred_lines: NDArray, 
    ...
    if num_gts == 0:
        ...
        for thr in thresholds:
            match_idx = np.full(num_preds, -1, dtype=np.int32)
            tp_fp_list.append((tp.copy(), fp.copy(), match_idx))
        return tp_fp_list
    ...
    for thr in thresholds:
        tp = np.zeros((num_preds), dtype=np.float32)
        fp = np.zeros((num_preds), dtype=np.float32)
        match_indices = np.full(num_preds, -1, dtype=np.int32)

        gt_covered = np.zeros(num_gts, dtype=bool)
        for i in sort_inds:
            if matrix_min[i] <= thr:
                matched_gt = matrix_argmin[i]
                match_indices[i] = matched_gt
        ...
        tp_fp_list.append((tp, fp, match_indices))
