...
from plugin.utils.mmdet_compat import AssignResult
...
class HungarianLinesAssigner:
    ...
    def __init__(self,
        ...
        self.cost = TASK_UTILS.build(cost)
    ...
    def assign(self,
        ...
        if track_info is not None:
            ...
            for j in range(cost.shape[0]):
                ...
                # Tweak the cost matrix here to force the G.T. assignment of the track queries
                    ...
                    if track_query_id < 0 or track_query_id >= cost.shape[1]:
                        # No valid target to force; leave row penalized.
                        cost[j] = LARGE_COST
                    else:
                        cost[j] = LARGE_COST
                        cost[:, track_query_id] = LARGE_COST
        ...
        try:
            ...
            raise RuntimeError(
                f'linear_sum_assignment failed (cost max={cost.max()}, '
                f'min={cost.min()}): {e}'
            ) from e
        ...
        return AssignResult(
            num_gts=num_gts,
            gt_inds=assigned_gt_inds,
            labels=assigned_labels), gt_permute_idx, mathced_reg_cost
