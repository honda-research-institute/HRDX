...
class VectorInstanceMemory(nn.Module):
    ...
    def __init__(self,
        ...
        super().__init__()
        self.max_number_ins = 3 * number_ins # initial capacity
    ...
    def init_memory(self, bs):
        ...
        self.mem_bank = torch.zeros((self.bank_size, bs, self.max_number_ins, self.dim_in), dtype=torch.float32, device=device)
        self.mem_bank_seq_id = torch.zeros((self.bank_size, bs, self.max_number_ins), dtype=torch.long, device=device)
        self.mem_bank_trans = torch.zeros((self.bank_size, bs,  3),dtype=torch.float32, device=device)
        self.mem_bank_rot = torch.zeros((self.bank_size, bs, 3, 3),dtype=torch.float32, device=device)
        self.batch_mem_embeds_dict = {}
        self.batch_mem_relative_pe_dict = {}
        self.batch_key_padding_dict = {}
        self.curr_rot = torch.zeros((bs,3,3),dtype=torch.float32, device=device)
        self.curr_trans = torch.zeros((bs,3),dtype=torch.float32, device=device)
        self.gt_lines_info = {}
        ...
        init_entry_length = torch.zeros(self.max_number_ins, dtype=torch.long, device=device)
        self.mem_entry_lengths = [init_entry_length.clone() for _ in range(bs)]

    def _ensure_capacity(self, required_ins: int):
        if required_ins < self.max_number_ins:
            return
        new_capacity = max(required_ins + 8, int(self.max_number_ins * 1.5))
        device = self.mem_bank.device
        old_cap = self.max_number_ins

        new_mem_bank = torch.zeros((self.bank_size, self.batch_size, new_capacity, self.dim_in), device=device)
        new_mem_bank[:, :, :old_cap] = self.mem_bank
        self.mem_bank = new_mem_bank

        new_seq_id = torch.zeros((self.bank_size, self.batch_size, new_capacity), dtype=torch.long, device=device)
        new_seq_id[:, :, :old_cap] = self.mem_bank_seq_id
        self.mem_bank_seq_id = new_seq_id

        new_rot = torch.zeros((self.bank_size, self.batch_size, 3, 3), dtype=torch.float32, device=device)
        new_rot.copy_(self.mem_bank_rot)
        self.mem_bank_rot = new_rot

        new_trans = torch.zeros((self.bank_size, self.batch_size, 3), dtype=torch.float32, device=device)
        new_trans.copy_(self.mem_bank_trans)
        self.mem_bank_trans = new_trans

        for i in range(self.batch_size):
            old = self.mem_entry_lengths[i]
            new_len = torch.zeros(new_capacity, dtype=torch.long, device=device)
            new_len[:old_cap] = old
            self.mem_entry_lengths[i] = new_len

        self.max_number_ins = new_capacity

    def update_memory(self, batch_i, is_first_frame, propagated_ids, prev_out,
                      num_tracks, seq_idx, timestep):
        device = self.mem_bank.device
        if is_first_frame:
            num_instances = len(propagated_ids)
            self._ensure_capacity(num_instances)
            mem_instance_ids = torch.arange(num_instances, device=device)
            track2mem_info = {i: i for i in range(num_instances)}
        else:
            ...
            for pred_i, propagated_id in enumerate(propagated_ids):
                if propagated_id < num_tracks:  # existing tracks
                    key = propagated_id.item()
                    if key in track2mem_info_prev:
                        track2mem_info[pred_i] = track2mem_info_prev[key]
                    else:
            ...
            self._ensure_capacity(num_instances)
            mem_instance_ids = torch.tensor([track2mem_info[item]
                                             for item in range(len(propagated_ids))],
                                            device=device).long()
    ...
    def trans_memory_bank(self, query_prop, b_i, metas):
        ...
        key_padding_mask = torch.ones((self.mem_len, num_track_ins), dtype=torch.bool, device=mem_embeds.device)
        ...
        relative_seq_idx = torch.zeros_like(mem_embeds[:, :, 0], dtype=torch.long, device=mem_embeds.device)
        relative_seq_idx[:valid_mem_len] = seq_id - mem_seq_ids[:valid_mem_len]
        relative_seq_idx = relative_seq_idx.clamp(min=0, max=self.cached_pe.shape[0] - 1)
