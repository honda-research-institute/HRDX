...
# Ensure backbone/neck implementations are registered
from mmdet.models.backbones import ResNet  # noqa: F401
from mmdet.models.necks import FPN  # noqa: F401
from plugin.utils.fp16_utils import force_fp32, auto_fp16
...
class BEVFormerBackbone(nn.Module):
    ...
    def __init__(self,
        ...
        if img_backbone:
            self.img_backbone = build_mmdet_module(img_backbone)
        if img_neck is not None:
            self.img_neck = build_mmdet_module(img_neck)
        ...
        pos_cfg = copy.deepcopy(positional_encoding)
        if isinstance(pos_cfg, dict):
            pos_cfg.setdefault('_scope_', 'mmdet')
        self.positional_encoding = build_mmdet_module(pos_cfg)
