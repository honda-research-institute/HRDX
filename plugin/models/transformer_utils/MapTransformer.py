...
from plugin.utils.registry import (TRANSFORMER_LAYER,
...
from plugin.utils.registry import TRANSFORMER

from plugin.utils.transformer import Transformer

from .CustomMSDeformableAttention import CustomMSDeformableAttention
from plugin.utils.mmdet_compat import inverse_sigmoid
        ...
        for layer in self.operation_order:
            if layer == 'self_attn':
                ...
                if attn_index == 1:
                    ...
                    if self.training and not memory_attn_used:
                        # Keep memory-attention parameters in the autograd graph
                        # even when a batch has no valid track queries.
                        zero = query.new_tensor(0.0)
                        for param in self.attentions[attn_index].parameters():
                            zero = zero + param.sum() * 0.0
                        query_bev = query_bev + zero
