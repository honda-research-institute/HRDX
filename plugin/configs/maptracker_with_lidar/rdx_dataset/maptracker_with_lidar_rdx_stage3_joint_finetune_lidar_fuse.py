...
lidar_data_root = './datasets/RDX/pointclouds'
num_cams = 6
...
train_pipeline = [
    ...
    dict(
        ...
        file_key='lidar_filepath',
        load_dim=4,
        use_dim=(0, 1, 2, 3),
        filter_roi=((-roi_size[0] / 2, roi_size[0] / 2),
                    (-roi_size[1] / 2, roi_size[1] / 2),
                    (-10.0, 10.0)),
        remove_close=True,
        close_radius=1.5,
        close_z=(-2.5, 2.5),
    ...
    dict(
        type='Collect3D',
        keys=['img', 'vectors', 'semantic_mask', 'points'],
        meta_keys=(
            'token', 'ego2img', 'sample_idx', 'ego2global_translation',
            'ego2global_rotation', 'img_shape', 'scene_name', 'lidar_filepath'))
]
...
load_from = "work_dirs/maptracker_with_lidar_rdx_stage2_warmup_lidar_fuse/latest.pth"
SyncBN = True
...
vis_backends = [
    ...
    dict(
        ...
        init_kwargs=dict(
            ...
            name='maptracker_rdx_stage3_lidar_fuse',
            tags=['stage_3', 'rdx', 'lidar']),
        define_metric_cfg=[
