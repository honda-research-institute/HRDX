...
model = dict(
    ...
    mem_len=4,
    mem_warmup_iters=-1,
    # LiDAR->BEV fusion (enabled)
    lidar_bev_cfg=dict(
        enabled=True,
        type='LidarPillarEncoder',
        out_channels=64,
        roi_size=roi_size,
        bev_h=bev_h,
        bev_w=bev_w,
...
train_pipeline = [
    ...
    dict(type='LoadLidarPointsFromFile', file_key='lidar_filename', load_dim=5, use_dim=(0, 1, 2, 3),
         filter_roi=((-roi_size[0]/2,roi_size[0]/2),(-roi_size[1]/2,roi_size[1]/2),(-20,20))),
    dict(type='FormatBundleMap'),
    dict(type='Collect3D', keys=['img', 'vectors', 'semantic_mask', 'points'], meta_keys=(
        'token', 'ego2img', 'sample_idx', 'ego2global_translation',
        'ego2global_rotation', 'img_shape', 'scene_name', 'lidar_filename'))
]
...
eval_config = dict(
    ...
    pipeline=[
        ...
        dict(type='LoadLidarPointsFromFile', file_key='lidar_filename', load_dim=5, use_dim=(0, 1, 2, 3),
             filter_roi=((-roi_size[0]/2,roi_size[0]/2),(-roi_size[1]/2,roi_size[1]/2),(-20,20))),
        dict(type='FormatBundleMap'),
        dict(type='Collect3D', keys=['vectors', 'points'], meta_keys=['token', 'ego2img', 'sample_idx', 'ego2global_translation',
...
match_config = dict(
    ...
    pipeline=[
        ...
        dict(type='LoadLidarPointsFromFile', file_key='lidar_filename', load_dim=5, use_dim=(0, 1, 2, 3),
             filter_roi=((-roi_size[0]/2,roi_size[0]/2),(-roi_size[1]/2,roi_size[1]/2),(-20,20))),
        dict(type='FormatBundleMap'),
        dict(type='Collect3D', keys=['vectors', 'semantic_mask', 'points'], meta_keys=['token', 'ego2img', 'sample_idx', 'ego2global_translation',
        'ego2global_rotation', 'img_shape', 'scene_name'])
    ],
    interval=1,
)
...
log_config = dict(
    ...
    hooks=[
        ...
        dict(
            type='MMDetWandbHook',
            init_kwargs={
                'project': 'maptracker-expts',
                'name': 'maptracker_nusc_newsplit_5frame_span10_stage3_joint_finetune_lidar_fuse',   # ✅ set a run name
                # optional: if you’re logging to a team/org
                # 'entity': 'your-team',
                # ✅ log the whole config (must be JSON-serializable)
            },
            interval=50,
            log_checkpoint=True,
...
load_from = "work_dirs/maptracker_nusc_newsplit_lidar_fuse_5frame_span10_stage2_warmup/latest.pth"
