...
num_epochs = 12#36
num_epochs_interval = num_epochs // 3
...
model = dict(
    ...
    mem_len=4,
    mem_warmup_iters=-1,
    # Enable aerial imagery fusion (AID4AD-style)
    sat_bev_cfg=dict(
        enabled=True,
        # Optional tuning knobs for the satellite encoder
        resunet_out_channels=64,
        down_hidden_dim=128,
        spatial_patch_dropout=dict(
            enabled=True,
            mask_ratio_range=(0.08, 0.16),
            num_patches_range=(1, 3),
            patch_hw_ratio_range=(0.05, 0.15),
            max_tries_per_patch=4,
        ),
    ),
    fusion_bev_cfg=dict(
        enabled=True,
        type='FusionBEV',
        num_heads=8,
        cam_channels=bev_embed_dims,
        gate_cfg=dict(
            enabled=True,
            apply_mode='pre_attn',
            in_mode='cam_aux_absdiff',
            hidden_channels=64,
            proj_channels=64,
            ...
            loss_schedule=dict(
                ...
                start_iter=25000,
                end_iter=75000,
                lambda_masked_gate_target=0.005,
                lambda_gate_after=5e-5,
                lambda_entropy_after=0.0,
            ),
            supervision=dict(
                enabled=False,
                csv_path='work_dirs/analysis/gate_targets_train_20260228_192503/gate_targets_train_delta.csv',
                target_key='g_star_combo',
                weight_key='gate_target_weight',
                token_key='token',
                loss_type='bce',
                eps=1e-6,
                target_clip_min=0.0,
...
optimizer = dict(
    ...
    paramwise_cfg=dict(
        custom_keys={
            ...
            # satellite branch + fuser
            'sat_encoder': dict(lr_mult=0.1),
            'sat_downsampler': dict(lr_mult=0.1),
            'fusion_bev': dict(lr_mult=0.5),
        }),
    weight_decay=1e-2)
...
load_from = "work_dirs/maptracker_rdx_stage2_warmup_aerial_fuse/latest.pth"
...
vis_backends = [
    ...
    dict(
        ...
        init_kwargs=dict(
            ...
            name='maptracker_RDX_aerial_fuse_stage3',
            tags=['stage_3', 'aerial_fuse']),
            #entity='honda-sv'
        define_metric_cfg=[
