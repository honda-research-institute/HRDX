...
_schema = import_module('plugin.datasets.map_utils.rdx_schema')
cat2id = dict(_schema.CAT2ID)
# model type
type = 'Mapper'
plugin = True
...
plugin_dir = 'plugin/'
[]
# img configs – still needed for dataset loading even though camera is unused
img_norm_cfg = dict(
...
img_top_crop_ratio = 0.36
img_data_root = './datasets/RDX/1by2.5x_jpegs'
num_cams = 6

num_gpus = 4
batch_size = 8
num_iters_per_epoch = 161725 // (num_gpus * batch_size)
num_epochs = 6
...
attribute_heads = {
    name: dict(
        ...
        applies_to=[cat2id[cls_name] for cls_name in schema['applies_to']],
        loss_weight=1.0,
    )
    for name, schema in _schema.ATTRIBUTE_SCHEMAS.items()
}
del _schema
...
meta = dict(
    ...
    use_radar=False,
    use_map=False,
    use_external=True,  # aerial imagery
...
model = dict(
    ...
    # ---- Aerial-only mode: bypass camera backbone, use aerial encoder directly ----
    aerial_only=True,
    sat_bev_cfg=dict(
        enabled=True,
        resunet_out_channels=64,
        down_hidden_dim=128,  # output channels = 128*2 = 256 = bev_embed_dims
    ),
    # No fusion needed – aerial features go directly to the neck
    fusion_bev_cfg=None,
    # Camera backbone is still instantiated (required by model init) but never used
    backbone_cfg=dict(
...
# Camera images are still loaded (dataset infra requires them) but not used by the model
train_pipeline = [
    dict(
        type='VectorizeMap',
    ...
    dict(type='Collect3D', keys=['img', 'vectors', 'semantic_mask', 'aerial_img'], meta_keys=(
...
eval_config = dict(
    ...
    aerial_crop_root='./datasets/RDX/aerial_ego_aligned',
    test_mode=True,
    ann_file='./datasets/RDX/val_1_every_5m.pkl',
    meta=meta,
    roi_size=roi_size,
    cat2id=cat2id,
    pipeline=[
...
match_config = dict(
    ...
    pipeline=[
        ...
        dict(type='LoadAerialImageFromFile', to_float32=True),
        dict(type='FormatBundleMap'),
        dict(type='Collect3D', keys=['vectors', 'aerial_img'], meta_keys=['token', 'ego2img', 'ego2cam', 'sample_idx', 'ego2global_translation',
...
# optimizer – only train aerial encoder + neck/seg/head; camera backbone is unused
# LR scaled by sqrt(4)=2x from 5e-4 due to larger effective batch (4 GPUs × 8 = 32 vs original 16)
optimizer = dict(
    type='AdamW',
    lr=1e-3,
    paramwise_cfg=dict(
        custom_keys={
            # Freeze camera backbone (unused in aerial_only mode)
...
find_unused_parameters = True  # camera backbone params are unused
...
vis_backends = [
    ...
    dict(
        ...
        init_kwargs=dict(
            ...
            name='maptracker_RDX_aerial_only_stage1',
            tags=['stage_1', 'aerial_only']),
        define_metric_cfg=[
