...
model = dict(
    ...
    freeze_bev=True,   # freezes camera backbone + seg_decoder (irrelevant here)
    track_fp_aug=False,
    use_memory=True,
    mem_len=4,
...
# optimizer – aerial encoder trains at base LR; camera backbone frozen at 0
# LR scaled by sqrt(4)=2x from 5e-4 due to larger effective batch
optimizer = dict(
    type='AdamW',
    lr=1e-3,
    paramwise_cfg=dict(
        custom_keys={
            ...
            # Slow down aerial encoder to preserve ImageNet features during warmup
...
load_from = "work_dirs/maptracker_rdx_stage1_bev_pretrain_aerial_only/latest.pth"
...
vis_backends = [
    ...
    dict(
        ...
        init_kwargs=dict(
            ...
            name='maptracker_RDX_aerial_only_stage2',
            tags=['stage_2', 'aerial_only']),
        define_metric_cfg=[
