...
The examples below use the legacy nuScenes / Argoverse2 configs to mirror the
upstream MapTracker. For the HRDX-flavored experiments (this repo's primary
benchmark), swap in the matching config under
`plugin/configs/maptracker/rdx_dataset/`,
`plugin/configs/maptracker_aerial_fuse/rdx_dataset/`,
`plugin/configs/maptracker_aerial_only/rdx_dataset/`, or
`plugin/configs/maptracker_knowledge_distillation/rdx_dataset/`, e.g.
...
HRDX commands follow the same three-stage protocol (Stage 1 BEV pretrain →
Stage 2 vector warmup → Stage 3 joint finetune); set `load_from` in each
config to point at the prior stage's checkpoint.
...
--option vis-gt
```
To generate video with aerial images and maps:
```bash
python tools/visualization/vis_per_frame.py <CONFIG> --data_path <PRED.pkl> --out_dir vis_local/nuscenes_aerial_fuse --option vis-pred --scene_id scene-0001 --aerial-root datasets/nuscenes/AID4AD_lidar_ego_referenced
```
