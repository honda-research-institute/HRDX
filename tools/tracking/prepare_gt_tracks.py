...
def find_matchings(src_masks, tgt_masks, thresh=0.1):
    ...
    for label, src_instances in src_masks.items():
        tgt_instances = tgt_masks.get(label, [])

        if len(src_instances) == 0 and len(tgt_instances) == 0:
            matchings[label] = ([], [])
            continue
        if len(src_instances) == 0:
            matchings[label] = ([], [-1 for _ in range(len(tgt_instances))])
            continue
        if len(tgt_instances) == 0:
            matchings[label] = ([-1 for _ in range(len(src_instances))], [])
            continue
...
def match_two_consecutive_frames(prev_data, curr_data, roi_size, origin, cfg):
    ...
    all_labels = set(prev_vectors.keys()) | set(curr_vectors.keys())
    prev2curr_vectors = dict()
    for label in all_labels:
        vecs = prev_vectors.get(label, [])
        if len(vecs) > 0:
    ...
    for label in all_labels:
        ...
        curr_label_vecs = curr_vectors.get(label, [])
        if len(curr_label_vecs) > 0:
            curr_vecs = torch.tensor(np.stack(curr_label_vecs))
            curr_vectors[label] = curr_vecs * roi_size + origin
        else:
            curr_vectors[label] = curr_label_vecs
...
def assign_global_ids(matchings_seq, vectors_seq):
    ...
    for t, vectors_t in enumerate(vectors_seq[1:]):
        ...
        for label, vectors in vectors_t.items():
            reverse_matching = matchings_seq[t].get(label, ([], []))[1]
            prev_label_ids = ids_seq[-1].get(label, {})
            id_mapping = dict()
            for i, _ in enumerate(vectors):
                if i < len(reverse_matching) and reverse_matching[i] != -1 and reverse_matching[i] in prev_label_ids:
                    prev_id = reverse_matching[i]
                    global_id = prev_label_ids[prev_id]
                else:
